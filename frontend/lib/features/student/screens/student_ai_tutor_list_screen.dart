...
import 'package:ieum/features/student/data/models/ai_tutor_session.dart';
          ...
          children: [
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                onRefresh: () async {
                  ref.invalidate(aiTutorActiveSessionsProvider);
                  ref.invalidate(studentProblemsProvider);
                  await ref.read(studentProblemsProvider.future);
                },
                child: async.when(
                  ...
                  data: (all) {
                    ...
                    // 진행 중(종료 안 된) 세션의 문제를 상단으로. (best-effort: 실패해도 목록은 보임)
                    final sessions =
                        ref.watch(aiTutorActiveSessionsProvider).valueOrNull ??
                            const <AiTutorSession>[];
                    final problemById = {
                      for (final p in all) p.problemId: p,
                    };
                    final active = <(AiTutorSession, StudentProblemModel)>[
                      for (final s in sessions)
                        if (problemById[s.problemId] != null)
                          (s, problemById[s.problemId]!),
                    ];
                    final activeIds = active.map((e) => e.$2.problemId).toSet();
                    final pending = items
                        .where((p) => !activeIds.contains(p.problemId))
                        .toList();
                    ...
                    void open(StudentProblemModel p) {
                      Navigator.of(context)
                          .push(
                            MaterialPageRoute(
                              builder: (_) => StudentAiTutorScreen(
                                problemId: p.problemId,
                                problemSummary: p.summary,
                              ),
                            ),
                          )
                          // 채팅을 시작/종료하고 돌아오면 '진행 중' 섹션 갱신.
                          .then((_) =>
                              ref.invalidate(aiTutorActiveSessionsProvider));
                    }
                    ...
                    return ListView(
                      ...
                      children: [
                        if (active.isNotEmpty) ...[
                          ...
                          _SectionLabel(shell: shell, text: '질문할 문제'),
                          const SizedBox(height: 10),
                        ],
                        for (final p in pending) ...[
                          _ProblemCard(
                              shell: shell, item: p, onTap: () => open(p)),
                          const SizedBox(height: 12),
                        ],
                        if (pending.isEmpty && active.isNotEmpty)
                          Padding(
                            padding: const EdgeInsets.only(top: 2),
                            child: Text('질문할 새 문제가 없어요.',
                                style: TextStyle(
...
/// 진행 중 AI 튜터 채팅 카드 — 연두 테두리 + 과목·요약 + 마지막 활동시간 + '이어서'.
class _ActiveChatCard extends StatelessWidget {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: shell.cardBackground,
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: AppColors.studentPoint, width: 1.5),
        ),
        child: Row(
          children: [
            ClipRRect(
              ...
              child: SizedBox(
                ...
                child: item.imageUrls.isNotEmpty
                    ? Image.network(
                        ApiConstants.resolveImageUrl(item.imageUrls.first),
                        fit: BoxFit.cover,
                        errorBuilder: (_, _, _) => _thumbFallback(shell),
                      )
                    : _thumbFallback(shell),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const Spacer(),
                      Text(
                        _timeAgo(session.updatedAt ?? session.createdAt),
                        style:
                            TextStyle(fontSize: 11.5, color: shell.hintColor),
                      ),
                    ],
  ...
  static Widget _thumbFallback(ShellTheme shell) => Container(
        color: shell.cardBorder.withValues(alpha: 0.3),
        alignment: Alignment.center,
        child: Icon(Icons.image_outlined, color: shell.hintColor, size: 22),
      );
