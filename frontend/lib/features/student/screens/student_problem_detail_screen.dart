...
class StudentProblemDetailScreen extends ConsumerWidget {
  const StudentProblemDetailScreen({
    super.key,
  ...
  /// 매칭완료·풀이완료·취소·만료 질문은 보기 전용(분류 수정 불가)으로 열고,
  /// 상태에 맞는 추천 동작(복습/다시 질문)만 노출한다.
  final bool readOnly;

  /// 만료·취소된 질문을 다시 강사 탐색으로 되돌린다(POST /matching/{id}/start).
  Future<void> _reRequest(BuildContext context, WidgetRef ref) async {
    try {
      await MatchingRepository().startMatching(problem.problemId);
            ...
            body: SafeArea(
              child: ListView(
                ...
                children: [
                  ...
                  // 보기 전용: 분류 수정 대신 상태별 추천 동작만 노출.
                  if (readOnly)
                    ..._buildRecommendation(context, ref, isDark)
                  else
                    SizedBox(
                      height: 52,
  ...
  /// 상태별 추천: 매칭완료·풀이완료 → AI 튜터로 복습, 취소·만료 → 다시 질문.
  List<Widget> _buildRecommendation(
      BuildContext context, WidgetRef ref, bool isDark) {
    final status = problem.status;
    final isReview = status == 'MATCHED' || status == 'RESOLVED';
    ...
    return [
      SizedBox(
        ...
        child: OutlinedButton.icon(
          ...
          icon: Icon(icon, size: 20, color: AppColors.studentPoint),
          label: Text(
            label,
            style: TextStyle(
              fontSize: 16,
...
/// 보기 전용 안내 배너 — 수정 불가임을 명확히 보여준다.
class _ViewOnlyBanner extends StatelessWidget {
  const _ViewOnlyBanner({required this.shell});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            Border.all(color: AppColors.studentPoint.withValues(alpha: 0.6)),
      ),
      child: Row(
        children: [
          Icon(Icons.visibility_outlined, size: 18, color: shell.titleColor),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              '확인 전용 화면이에요. 이 질문은 수정할 수 없어요.',
              style: TextStyle(
                fontSize: 13,
      ...
      child: Stack(
        children: [
          Positioned.fill(
            child: ClipRRect(
              borderRadius: BorderRadius.circular(12),
              // 문제 전체가 잘리지 않게 contain. 남는 여백은 배경색으로 채워 깔끔하게.
              child: Container(
