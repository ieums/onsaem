 ...
 * 어떤 Problem도 참조하지 않는 고아가 된다 → 주기적으로 스토리지에서 삭제.
...
public class ProblemImageCleanupScheduler {
    ...
    public void cleanupOrphanDetectionImages() {
        List<String> orphans = detectionCache.sweepExpired();
        if (!orphans.isEmpty()) {
            log.info("[ImageCleanup] 미선택 만료 업로드 고아 이미지 {}건 삭제", orphans.size());
            imageStorageService.deleteAll(orphans); // best-effort (실패해도 예외 안 던짐)
        }
    }
}
